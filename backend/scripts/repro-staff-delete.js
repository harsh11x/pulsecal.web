...
(async () => {
  ...
  const staffEmail = `staff.test.${Date.now()}@pulsecal.local`;
  ...
  console.log('CREATE_BODY', JSON.stringify(createBody).slice(0, 800));

  const delRes = await fetch(`${API}/users/${dbUser.id}/status`, {
    method: 'PATCH',
    headers: {
      Authorization: `Bearer ${token}`,
      'Content-Type': 'application/json',
  ...
  console.log('DELETE_BODY', JSON.stringify(delBody).slice(0, 500));

  // cleanup created firebase user if any
  if (createRes.ok && createBody?.data?.email) {
    try {
      const fu = await admin.auth().getUserByEmail(staffEmail);
      await admin.auth().deleteUser(fu.uid);
      console.log('CLEANED_FIREBASE', staffEmail);
    } catch (e) {
  ...
  if (createBody?.data?.id) {
    try {
      await prisma.$executeRawUnsafe(`DELETE FROM users WHERE id = $1`, createBody.data.id);
      console.log('CLEANED_DB', createBody.data.id);
    } catch (e) {
