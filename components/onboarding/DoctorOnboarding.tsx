...
export default function DoctorOnboarding() {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      // BYPASS PAYMENT - Create clinic directly
      toast.loading('Creating clinic...')

      const clinicResponse: any = await apiService.post("/api/v1/clinics", {
      ...
      if (clinicResponse.data?.clinic?.id || clinicResponse.data?.id) {
        const clinicId = clinicResponse.data?.clinic?.id || clinicResponse.data?.id
